//
//  FITEventsViewController.m
//  TakeTwo
//
//  Created by Anthony Urena on 5/17/14.
//  Copyright (c) 2014 Families In Transition. All rights reserved.
//
#import "FITEventsViewController.h"

@interface FITEventsViewController ()

@end

@implementation FITEventsViewController
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [self loadWebViewContent:EVENTS_PAGE_URL];
}
...
- (void)loadWebViewContent:(NSString*)webPage
{
    ...
    NSString *content = [self saveMainPage:@"events.html" FromContent:data]; //Define different .html pages later?
    
    //NSString *contentUrl = [NSString stringWithFormat:@"%@%@", self.mTemporaryUrl, @"/events.html"];
...
- (NSString*) saveMainPage:(NSString*)fileName FromContent:(NSString*)pageSource
{
    ...
    if(startContent.location == NSNotFound)
    {
        ...
        //For loading local html/css files...
        NSBundle *mainBundle = [NSBundle mainBundle];
        NSURL *homeIndexUrl = [mainBundle URLForResource:@"events" withExtension:@"html"];
